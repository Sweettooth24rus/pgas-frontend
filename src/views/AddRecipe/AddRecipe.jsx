    ...
    if (types){
        return(
            <div className={'AddRecipe'}>
                <p className={'AddRecipe__title'}>Заявка за 1 семестр 2022 года</p>
                <table bordercolor="black" rules="rows" width="100%">
                                        <tr>
                                        <td>Баллы</td>
                                        <td>Текстовое описание документа</td>
                                        <td>Просмотр файлов</td>
                                        </tr>
                                        <tr>
                                                                <td> </td>
                                                                                                        <td>Документ №1</td>
                                                                                                        <td><Input type={'file'} onChange={e => saveImage(e.target.files[0])} /></td>
                                        </tr>
                                        <tr>
                                                                                                        <td> </td>
                                                                                                                                                <td>Документ №2</td>
                                        ...
                                        <p>
                                        <Link style={{textDecoration: 'none', color: 'green'}} to={'/pickle/add'}>Закрыть </Link>
                                        <Link style={{textDecoration: 'none', color: 'green'}} to={'/pickle/add'}> Сохранить </Link>
                                        <Link style={{textDecoration: 'none', color: 'green'}} to={'/pickle/add'}> Сохранить как черновик</Link>

                                        </p>
                {/* <div style={{display: 'flex', flexDirection: 'row'}}>
                    <div style={{display: 'flex', flexDirection: 'column'}}>
                <Input type={'file'} onChange={e => saveImage(e.target.files[0])} />
                <TextField className={'AddRecipe__input'} style={{marginTop: 20}} variant={'outlined'} label={'Название статьи'} onChange={e => setRecipe({...recipe, name: e.target.value})} />
